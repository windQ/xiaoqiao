...
class Controller_Default extends Controller_Abstract
{
    ...
    function actionNewsdetail()
    {
    	...
    	//上一篇
    	$prev_news = News::find( 'id < ? AND `column` = ? AND sub_column_id = ?', $news_detail->id, $column, $sub_column )->order( 'create_time DESC, id DESC' )->getOne();
    	$next_news = News::find( 'id > ? AND `column` = ? AND sub_column_id = ?', $news_detail->id, $column, $sub_column )->order( 'create_time ASC, id ASC' )->getOne();
    	$this->_view[ 'column' ]    = $column;
    	$this->_view[ 'sub_column' ] = $sub_column;
    ...
    function actionPhotos()
    {
    	...
    	switch ( $column )
    	{
    		...
    		case '6':
    			...
    			$prod_obj = $column_for_data = Product::find( '`column` = ?', $column );
    			if( $sub_id != 0 )
    			{
    				$prod_obj->where( 'sub_column_id = ?', $sub_id );
    			}
    			$column_for_data = $prod_obj->order( 'create_time DESC, id DESC' )->getAll();
